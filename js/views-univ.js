...
(function (A) {
  ...
  A.route('/univ', ['univ'], (_, me) => {
    const ps = S.pshipsOf(me), running = ps.filter(x => ['requested', 'reserved', 'proposal'].includes(x.status)).sort((a, b) => b.createdAt - a.createdAt);
    ...
    return V.head(`Halo, ${A.esc(pr.contactName || me.name)}`, `Ringkasan aktivitas KKN ${A.esc(me.name)}`, `<a class="btn" href="#/univ/discover">${A.ic('compass')} Jelajahi desa</a>`) +
      `<div class="g4">${V.stat('heart-handshake', 'c-amber', ps.filter(x => ['reserved', 'proposal'].includes(x.status)).length, 'Partnership berjalan')}${V.stat('file-text', 'c-blue', ps.filter(x => x.proposal?.status === 'submitted').length, 'Proposal dikirim')}${V.stat('circle-check', 'c-purple', ps.filter(x => x.status === 'matched').length, 'Matched')}${V.stat('compass', 'c-green', avail.length, 'Kebutuhan tersedia')}</div>
      <div class="g32 mt16"><div class="col">
        <div class="card"><div class="row sp"><h3 class="nb">Kebutuhan desa terbaru</h3><a class="xs b" style="color:var(--green-500)" href="#/univ/discover">Lihat semua</a></div><div class="list mt12">${avail.slice(0, 3).map(p => `<a class="item click" href="#/univ/problem/${p.id}">${A.photo(p.id, 't')}<div class="grow"><div class="b">${A.esc(p.title)}</div><div class="xs mu">${A.esc(S.user(p.desaId).name)} · ${A.esc(p.city)} · ${p.duration} bulan</div><div class="chips mt8">${A.chipTag(p.category, 'blue')}</div></div>${A.ic('chevron-right', 'mu')}</a>`).join('') || A.empty('compass', 'Belum ada kebutuhan tersedia.')}</div></div>
        <div class="card"><h3>Partnership & proposal</h3>${running.length ? `<div class="list">${running.map(r => { const d = V.deadline(r); return `<a class="item click" href="#/partnerships/${r.id}"><div class="grow"><div class="b">${A.esc(V.problemTitle(r))}</div><div class="xs mu">${A.esc(S.user(r.desaId).name)}</div><div class="bar thin mt8"><i style="width:${V.progress(r)}%"></i></div></div><div class="center">${A.tag(r.status)}<div class="xs mu mt4">${d ? `${A.ic('clock')} ${A.cd(d.end)}` : '—'}</div></div></a>`; }).join('')}</div>` : A.empty('heart-handshake', 'Belum ada partnership berjalan.')}</div>
      </div><div class="col">
  ...
  /* ================= discovery ================= */
  const VISIBLE = ['available', 'requested', 'reserved', 'proposal'];
  const list = me => {
    ...
    const rows = S.data.problems.filter(p => VISIBLE.includes(p.status) || (f.all && ['matched', 'expired'].includes(p.status)))
      ...
      .filter(p => !f.cat || p.category === f.cat).filter(p => !f.city || p.city === f.city).filter(p => !f.dur || p.duration <= +f.dur);
  ...
  const results = me => {
    ...
    const html = rows.map(p => {
      ...
      return `<div class="card tight ${locked ? 'locked' : 'click'}" data-act="openUP" data-id="${p.id}" style="cursor:pointer"><div class="row">${A.photo(p.id, 't')}<div class="grow"><div class="row wrap"><b style="font-size:16px">${A.esc(p.title)}</b>${mine ? A.tag(mine.status) : ''}${!mine && p.status !== 'available' && !locked ? A.tag(p.status) : ''}</div>
        <div class="sm mu mt4">${A.ic('building-community')} ${A.esc(S.user(p.desaId).name)} · ${A.ic('map-pin')} ${A.esc(p.city)} · ${A.ic('clock')} ${p.duration} bulan</div>
  ...
  A.route('/univ/discover', ['univ'], (_, me) => {
    ...
    return V.head('Jelajahi Desa', 'Temukan kebutuhan desa yang bisa dibantu tim Anda') +
      ...
      <div class="row wrap mt12"><select data-change="fSel" data-k="cat" style="width:auto"><option value="">Semua kategori</option>${A.CATEGORIES.map(c => `<option ${f.cat === c ? 'selected' : ''}>${c}</option>`).join('')}</select>
        ...
        <select data-change="fSel" data-k="dur" style="width:auto"><option value="">Semua durasi</option><option value="1" ${f.dur === '1' ? 'selected' : ''}>≤ 1 bulan</option><option value="2" ${f.dur === '2' ? 'selected' : ''}>≤ 2 bulan</option><option value="3" ${f.dur === '3' ? 'selected' : ''}>≤ 3 bulan</option></select>
      ...
      <div class="card soft tight mt12 sm">${A.ic('info-circle')} Kebutuhan berwarna <b>abu-abu</b> sedang diajukan atau berjalan bersama universitas lain. Kebutuhan itu terbuka kembali jika kerja samanya ditolak atau berakhir.</div>
  ...
  A.route('/univ/problem/:id', ['univ'], ({ id }, me) => {
    ...
    let cta;
    if (mine) cta = `<a class="btn" href="#/partnerships/${mine.id}">Buka partnership ${A.ic('arrow-right')}</a>`;
    else if (p.status === 'available') cta = `<button class="btn" data-act="applyPship" data-id="${id}">Ajukan partnership ${A.ic('arrow-right')}</button>`;
