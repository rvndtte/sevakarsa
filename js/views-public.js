...
(function (A) {
  ...
  const story = [['01', 'Available', 'tg-green', 'Desa mengajukan kebutuhan', 'Desa membuat profil, lalu mempublikasikan masalah beserta kompetensi yang dibutuhkan. Kebutuhan tampil terbuka bagi semua universitas.'], ['02', 'Diajukan', 'tg-blue', 'Universitas mengajukan', 'Universitas menjelajah kebutuhan desa lalu mengajukan kerja sama dengan kuota kelompok. Desa menyetujui atau menolak sekali; kebutuhan terkunci untuk pihak lain selama menunggu.'], ['03', 'Disetujui', 'tg-amber', 'Kontak & kesepakatan', 'Kontak kedua pihak terbuka dan diskusi berlanjut di WhatsApp. Koordinator KKN mengirim konfirmasi kesepakatan tiap kelompok, desa tinggal menekan Sesuai.'], ['04', 'Aktif', 'tg-purple', 'KKN & dokumentasi', 'Kerja sama Aktif. Pelaksanaan, laporan, dan arsip dokumentasi tersimpan sebagai riwayat desa.']];
  ...
  A.route('/', null, () => {
    ...
    <div class="hero"><div class="pubnav"><a class="brand" href="#/" style="padding:0">${A.ic('leaf')}SumbangRuang</a>
      ...
      <div class="hg"><div class="rise"><span class="eyebrow">${A.ic('sparkles')}Platform KKN Desa & Universitas</span>
        ...
        <div class="hpic rise d2"><div class="scene" aria-hidden="true"></div><div class="img">${hills}<img src="https://images.unsplash.com/photo-1716731049987-c5ec344a9c80?auto=format&fit=crop&w=1000&h=1000&q=80" alt="Pemandangan udara permukiman desa dengan atap genteng dan sawah" loading="eager" onerror="this.remove()"></div><div class="blob"></div>
          ...
          <div class="float f2">${A.ic('heart-handshake', 'lm')}<div><div class="b sm">Aktif</div><div class="xs mu">Desa ⇄ Universitas</div></div></div>
    ...
    <div class="section" id="benefit"><span class="kick">Manfaat</span><h2>Manfaat untuk semua pihak</h2>
      <div class="g2 mt24">
        <div class="card perk"><div class="perkh"><div class="ib c-green">${A.ic('home-heart')}</div><h3>Untuk desa</h3></div><div class="col gap8">${perks(['Masalah terdengar oleh banyak kampus', 'Solusi sesuai kebutuhan nyata desa', 'Anda cukup menyetujui, diskusi teknis lewat WhatsApp', 'Riwayat & dokumentasi tersimpan rapi'])}</div></div>
    ...
    <div class="section" id="faq"><span class="kick">FAQ</span><h2>Pertanyaan umum</h2>
      <div class="col gap8 mt24">${[['Siapa yang memverifikasi akun?', 'Super Admin memeriksa dokumen pendukung dalam 1–2 hari kerja sebelum akun desa atau universitas aktif penuh.'], ['Apakah diskusi dilakukan di platform?', 'Tidak. Setelah desa menyetujui kerja sama, kontak kedua pihak terbuka dan diskusi teknis dilakukan lewat WhatsApp. Platform hanya membuka kontak dan mencatat hasil kesepakatan.'], ['Apa arti status Available, Diajukan, Disetujui, Aktif?', 'Available: terbuka untuk pengajuan. Diajukan: menunggu keputusan desa (kedaluwarsa jika 7 hari tanpa respons) dan kebutuhan terkunci untuk universitas lain. Disetujui: kontak terbuka, koordinator berdiskusi dengan desa. Aktif: desa menyatakan kesepakatan sesuai. Jika ditolak, dibatalkan, atau kedaluwarsa, kebutuhan terbuka kembali.']].map(f => `<details class="card tight faq"><summary class="b">${f[0]}${A.ic('chevron-down', 'chev')}</summary><p class="mu mt8">${f[1]}</p></details>`).join('')}</div></div>
